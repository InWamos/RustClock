macro_rules ! set_clocks { ($ ($ (# [$ m : meta]) * $ k : ident : $ v : expr ,) *) => { { # [allow (unused)] struct Temp { $ ($ (# [$ m]) * $ k : Option < crate :: time :: Hertz > ,) * } let all = Temp { $ ($ (# [$ m]) * $ k : $ v ,) * } ; crate :: rcc :: set_freqs (crate :: rcc :: Clocks { hclk1 : all . hclk1 , hclk2 : all . hclk2 , hclk3 : all . hclk3 , hsi_div488 : all . hsi_div488 , lse : all . lse , pclk1 : all . pclk1 , pclk1_tim : all . pclk1_tim , pclk2 : all . pclk2 , pclk2_tim : all . pclk2_tim , pll1_q : all . pll1_q , pllsai1_q : all . pllsai1_q , rtc : all . rtc , sys : all . sys , }) ; } } ; }
#[allow(unused)]
...
macro_rules! foreach_interrupt {
    ($($pat:tt => $code:tt;)*) => {
        ...
        __foreach_interrupt_inner!((SPDIFRX,spdifrx,SPDIFRX,GLOBAL,SPDIF_RX));
...
macro_rules! foreach_peripheral {
    ($($pat:tt => $code:tt;)*) => {
        ...
        __foreach_peripheral_inner!((spdifrx,SPDIFRX));
